...
try {
    ...
    foreach ($sheetData as $row) {
        ...
        if (isset($row['A']) && trim($row['A']) == 'Item No.') {
            continue; 
        }
        
        if (empty($row['A'])) {
            echo json_encode(['success' => false, 'message' => 'ItemCode is empty.']);
            exit;
        }
        
        // Process the data into arrays
        $itemCodes[] = $row['A'];  // Column A - Item No.
        $itemNames[] = $row['B'];  // Column B - Item Description
        $itemUoms[] = $row['C'];   // Column C - Purchasing UoM
        $assetQuantities[] = $row['D'];  // Column D - Asset Quantity
        $quantities[] = $row['E'];   // Column E - Asset Quantity Request
        $alasan[] = $row['F'];   // Column F - Alasan
        $kondisiAssets[] = $row['G'];  // Column G - Kondisi Aset
        $keteranganAsset[] = $row['H']; // Column H - Keterangan Asset
        $assetConditionOk[] = $row['I'];  // Column I - Asset Condition OK
        $assetConditionNonOk[] = $row['J']; // Column J - Asset Condition Non OK
    }
