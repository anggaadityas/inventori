...
$id=$_GET['id'];
$sqlheader = "SELECT *,convert(char(10),reqtb_date,126) req_date,convert(char(10),reqtb_user_verifikasi_date,126) date_verifikasi FROM header_tb a
...
if($rowheader['reqtb_destination_approve'] == 'Approved'){
...
<span style="font-size:18px;"><b>* View Verifikasi Permintaan Transfer Putus Store <?php echo $rowheader['reqtb_code']; ?></b></span>
<br><br><br>
...
<?php
        ...
        $sqldetail = "SELECT *,convert(char(10),tbitem_expired,126) expired FROM detail_tb where header_idtb='$id'";
        ...
        $no++;
        $qtyver =number_format($rowdetail['tbitem_qty_verifikasi'],2,'.',',');
        if($qtyver == ''){
            $fixqtyver=0;
        }else{
            $fixqtyver=number_format($qtyver,2,'.',',');
        }
        $selisi =  number_format($fixqtyver,2,'.',',') - number_format($rowdetail['tbitem_qty_approve'],2,'.',',');
    ?>
    <tr>
              ...
              <td align="left"><?php echo number_format($rowdetail['tbitem_qty_approve'],2,'.',','); ?>
              <input type='hidden' id='qtyapprove_<?php echo $no; ?>' name='qtyapprove[<?php echo $rowdetail['id_detailtb']; ?>]' value="<?php echo number_format($rowdetail['tbitem_qty_approve'],2,'.',','); ?>" style='width:70px' class="form-control" autocomplete="off" readonly required/>
            </td> 
              <td align="left">
                  <input type='text' id='qtygood_<?php echo $no; ?>' name='qtyverifikasi_good[<?php echo $rowdetail['id_detailtb']; ?>]' value="<?php echo number_format($rowdetail['tbitem_qty_verifikasi'],2,'.',','); ?>" style='width:70px' class="qtygood" autocomplete="off" required/>
                </td>
              <!-- <td><?php echo $rowdetail['tbitem_item_condition_approve']; ?></td> -->
              <!-- <td><select name="kondisi[]" id="kondidi_0" required>
                <option value="">Pilih Kondisi</option>
                    <option value="Bagus" <?php if($rowdetail['tbitem_item_condition_verifikasi']=="Bagus") echo "selected"; ?>>Bagus</option>
                    <option value="Tidak Bagus" <?php if($rowdetail['tbitem_item_condition_verifikasi']=="Tidak Bagus") echo "selected"; ?>>Tidak Bagus</option>
                </select></td> -->
                <td>
                <input type='text' id='qtyverifikasi_<?php echo $no; ?>' name='qtyverifikasi[<?php echo $rowdetail['id_detailtb']; ?>]' style='width:70px' value="<?php echo number_format($selisi,2,'.',','); ?>" class="qtyverifikasi" autocomplete="off" readonly required/>
                </td>
                <td><?php echo $rowdetail['expired']; ?></td>
              <td align="left"><?php echo $rowdetail['tbitem_remarks_approve']; ?></td>
              <td align="left"><textarea type='text' class="form-control keterangan_barang"  oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "110" id='keteranganbarang_0' name='keterangan_barangverifikasi[<?php echo $rowdetail['id_detailtb']; ?>]' cols="30" rows="5"><?php echo $rowdetail['tbitem_remarks_verifikasi']; ?></textarea></td>
              </td>
            </tr>
    ...
    ?>
        ...
        <div class="form-group row">
                ...
                <div class="col-sm-12">
               <textarea name="note_request_verifkasi" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "110" class="form-control note_request_verifkasi"  cols="10" rows="5"><?php echo $rowheader['reqtb_user_verifikasi_note']; ?></textarea>
                </div>
            </div>
        ...
        <option value="Verifikasi" selected>Disetujui</option>
         <option value="Reject" <?php if($rowheader['reqtb_user_verifikasi']=="Reject") echo "selected"; ?>>Ditolak</option>
        </select>
        <br>
...
<script>
    ...
    $('.qtygood, .qtyverifikasi').inputmask({
    alias:"decimal",
    digits:2,
    repeat:13,
        ...
        swal.fire({
            ...
            showLoaderOnConfirm: true,
                preConfirm: function() {
                    ...
                    $.ajax({
                            url: "viewverifikasirequesttbsproses.php",
                            type: "POST",
...
$(document).on('change', '#status_request', function() {
  ...
  if(status_request =='Approved'){
    $(".qtyverifikasi").prop('required',true);
  }else if (status_request =='Reject') {
    $(".qtyverifikasi").prop('required',false);
